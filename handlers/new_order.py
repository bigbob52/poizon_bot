...
async def ask_item_size(message: Message, state: FSMContext):
    ...
    if not any(url.startswith(prefix) for prefix in allowed_prefixes):
        await bot_msg.edit_text(
            ...
            "Пожалуйста, убедитесь, что ссылка скопирована с официальной площадки Poizon.\n\n"
            "<i>Пример корректной ссылки:</i>\n"
            "https://dw4.co/t/A/1stiy6Mgg\n\n"
    ...
    if user.username:
        await callback.message.edit_text(
            text=(
                ...
                f"Наш менеджер уже получил уведомление и свяжется с вами в течение часа для уточнения деталей. Пожалуйста, ожидайте.\n\n"
